...
function regress_period(p, y, t, ω)
    A, B, C, σ = p
    ll = sum(logpdf.(Normal.(A * cos.(ω * t) + B * sin.(ω * t) .+ C, σ), y))
    return ll
end

ub = [10.0, 10.0, 10.0, 10.0]
lb = [-10.0, -10.0, -10.0, 0.1]
init_p = [0.0, 0.0, 0.0, 5.0]

optim_out = optimize(p -> -regress_period(p, dat.gauge, 1:nrow(dat), (2 * π)/365), lb, ub, init_p)
...
## Example: Daily Periods

If we want to look at the tide gauge data with a daily period, then 
$$\begin{gathered}
2\pi / \omega = 365 \\
\omega = 2\pi / 365
\end{gathered}$$

and we get: `{julia} mle`.
...
$$\begin{aligned}
\hat{A} &= \frac{2}{T} \sum_t y_t \cos(\omega_j t) \\
\hat{B} &= \frac{2}{T} \sum_t y_t \sin(\omega_j t) \\
\hat{C} &= \bar{Y} = \frac{1}{T} \sum_t y_t.
\end{aligned}$$
...
![Example of a Periodogram](https://vlyubchich.github.io/tsar/l13_spectral_files/figure-html/fig-airpassangersFFT-1.png){width=55%}

::: {.caption}
Source: [Time Series Analysis, Chapter 11](https://vlyubchich.github.io/tsar/l13_spectral.html)
:::
