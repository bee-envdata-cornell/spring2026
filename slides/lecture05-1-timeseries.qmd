...
optim_out = optimize(p -> -regress_period(p, dat.gauge, 1:nrow(dat), (2 * π) / 292), lb, ub, init_p)
mle = round.(optim_out.minimizer; digits=3)
```


If we want to look at the tide gauge data  (which has an hourly sampling period and 8760 samples) with a monthly period, then 
$$\begin{gathered}
2\pi / \omega = 8760 / 30 = 292 \\
\omega = 2\pi / 292
\end{gathered}$$

and we get: $[A, B, C, \sigma$] = $`{julia} mle`.
