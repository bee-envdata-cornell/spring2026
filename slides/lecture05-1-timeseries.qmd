...
$$\begin{aligned}
\mathbb{E}[Y_{t+h} | Y_t = y_t] &= \mathbb{E}[\alpha \sum_{k=0}^{t-1} \rho^k + \sum_{k=0}^{t-1} \varepsilon_{t-k} \rho^k + \rho^t y_0 | Y_t = y_t] \\
&= \alpha \sum_{k=0}^{t-1} \rho^k + \rho^t y_0
\end{aligned}$$

Under what condition will $\mathbf{y}$ be stationary?

::: {.fragment .fade-in}
Stationarity requires $| \rho | < 1$ and $\alpha = 0$.
:::
