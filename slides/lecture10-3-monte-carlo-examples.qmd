...
#| echo: true
#| code-fold: true
#| label: fig-mc-estimate-pilot-airshed
...
$\hat{\sigma}_n^2 =$  `{julia} round(var(mean_SO2 .> 0.14); digits=2)`
...
::: {.column width=50%}
MCSE: `{julia} round(std_mc_out[end]; digits=3)`

Estimate: `{julia} round(avg_mc_out[end]; digits=2)` $\pm$ `{julia} round(1.96 * std_mc_out[end]; digits=3)`
:::
::::
...
p = plot(avg_mc_out, xlabel="Monte Carlo Samples", ylabel="Damages (\$)", left_margin=3mm, legend=:false, ribbon=1.96*std_mc_out, fillalpha=0.3, linewidth=2, fillcolor=:red, right_margin=5mm, minorgrid=true)
yaxis!(p, yminorticks=5)
ylims!((0.5e4, 2e4))
plot!(p, size=(600, 450))
```
