...
pretty_table(DataFrame(Parameters=parnames, IID=θ_iid_exp, AR=θ_ar_rd); backend=:markdown)
```
...
plot!(time_obs, ebm_boot[:, 1], lw=1, label="Bootstrapped Realization")
plot!(time_obs, ebm_boot[:, 2], lw=1, label=false)
plot!(time_obs, ebm_boot[:, 3], lw=1, label=false)
plot!(size=(1200, 500))
```
