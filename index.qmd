---
title: "{{< var course.title >}}"
details: |
    This course focuses on probabilistic modeling of environmental data-generating processes, including simulation methods and model evaluation and selection.
subtitle: "BEE 4850/5850<br>Spring 2026<br>Cornell University"  
page-layout: full
course-logo: _assets/logos/envdata_logo.png
format:
    html:
        template-partials: 
            - _assets/partials/title-block.html
---

::: {.home}

:::: {.grid .course-details}

::: {.g-col-12 .g-col-sm-6 .g-col-md-3}
### Instructor

- {{< fa sharp user title="Name" >}} [[{{< var instructor.name_no_title >}}]({{< var instructor.url >}})]{.ms-1}
- {{< fa sharp envelope title="Email" >}} [[{{< var instructor.email >}}](mailto:{{< var instructor.email >}})]{.ms-1}
- {{< fa sharp building title="Office" >}} [{{< var instructor.office >}}]{.ms-2}
:::

::: {.g-col-12 .g-col-sm-6 .g-col-md-3}
### TA


- {{< fa sharp user title="Name" >}} [{{< var TA.name >}}]{.ms-1}
- {{< fa sharp envelope title="Email" >}} [[{{< var TA.email >}}](mailto:{{< var TA.email >}})]{.ms-1}
- {{< fa sharp building title="TA Office" >}} [TBD]{.ms-2}
:::

::: {.g-col-12 .g-col-sm-6 .g-col-md-3}
### Course details

- {{< fa sharp calendar title="Class Days" >}} {{< var course.days >}}
- {{< fa sharp clock title="Class Time" >}} {{< var course.time >}}
- {{< fa sharp university title="Class Location" >}} {{< var course.location >}}
:::

::: {.g-col-12 .g-col-sm-6 .g-col-md-3 .contact-policy}
### Contacting staff

- {{< fa sharp door-open title="Office Hours" >}}[(Instructor) MTh 10-11]{.ms-1}
- {{< fa sharp door-open title="TA Office Hours" >}}[(TA) TBD]{.ms-1}
- [{{< fa sharp comments title="Ed Discussion" >}}[Ed Discussion]{.ms-1}](https://edstem.org)

:::

::::


## Software Tools

- I will teach this course using the [Julia](https://julialang.org) programming language. Julia is a modern, free, open source language which is designed for scientific computing. No prior knowledge of Julia is required. My recommendation is to use [Visual Studio Code](https://code.visualstudio.com/) with the [official Julia extension](https://code.visualstudio.com/docs/languages/julia) for coding. However, you can use any programming language you would like to complete the assignments and the final project.
- Assignments will be distributed using [GitHub Classroom](https://github.com). You should create a GitHub account, but we will walk through how to use it. 